"""
consumers.py — WebSocket consumer for real-time ISL prediction.

Flow:
  Browser captures webcam frame → sends as binary (JPEG bytes) over WS
  Consumer runs ISLEngine.process_frame() → sends back JSON result
  Browser updates UI with letter / sentence / confidence
"""
...
class PredictorConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        # ── Binary message: webcam frame (JPEG bytes) ─────────────────────────
        if bytes_data:
        ...
        if text_data:
            ...
            if cmd == "SET_MODE":
                ...
                # ENTER: include matched phrase for TTS
                if cmd == "ENTER":
